...
为避免人物关联层抢过四色建筑信道的视觉重点，进一步调整为：

- 取消选中线的双层高亮描边；
- 选中线改为 RGBA `(55, 125, 165, 135)`、线宽 `2.25`；
- 未选中线继续使用虚线，改为 RGBA `(95, 102, 110, 80)`、线宽 `1.0`；
- 虚线间隔、12 条显示预算、10 Hz 刷新以及实时屋顶 `+4 cm` 端点全部保持不变。

运行检查结果：移动 100、卡死 0、连接 100、旧悬浮层可见 0。相比上一版，本次还删除了一次选中线绘制，因此绘制负担只会降低。

![弱化后的人物—基站关联线](../Evidence/InvestorDelivery/10_person_station_association_subtle_2026-08-12.png)
