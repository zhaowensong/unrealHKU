...
- 已补充可重复的 UnrealMCP 执行器与只读资产审计脚本。2026-07-15 对当前
  `/Game` 下 7311 个资产的实际审计结果是 `candidate_asset_count=0`，这证明官方
  City Sample Crowds 尚未进入工程，而不只是“内容浏览器里没看到”。
...
先在 UnrealMCP 控制面板确认 13377 端口处于 `Running`，然后在项目根目录运行：

```powershell
python Scripts/OpenMassCrowd/run_unreal_python_via_mcp.py `
  --file Scripts/OpenMassCrowd/audit_city_sample_crowds.py
```

脚本只查询 Asset Registry，不创建、修改或保存资产。官方资源迁入后再次运行，
输出中的 `candidate_paths_by_class` 将作为六个人物变体配置的真实路径依据，禁止
凭经验猜测 Blueprint、SkeletalMesh 或 Data Asset 名称。
