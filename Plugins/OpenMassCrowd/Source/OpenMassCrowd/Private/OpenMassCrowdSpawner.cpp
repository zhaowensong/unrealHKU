...
constexpr uint8 InvestorSelectedLinkRed = 24;
constexpr uint8 InvestorSelectedLinkGreen = 82;
constexpr uint8 InvestorSelectedLinkBlue = 138;
constexpr uint8 InvestorSelectedLinkAlpha = 170;
...
void AOpenMassCrowdSpawner::DrawInvestorAssociationVisuals()
{
    ...
    // Keep the selected person-to-station link translucent, but use a deeper
    // blue than the surrounding gray dashed links so selection reads clearly
    // without becoming another bright telecom-ray layer.
    const FLinearColor SelectedColor = FLinearColor::FromSRGBColor(
        FColor(
            InvestorSelectedLinkRed,
            InvestorSelectedLinkGreen,
            InvestorSelectedLinkBlue,
            InvestorSelectedLinkAlpha));
